        ...
        console.log(2);
         console.log(data);
            for (var j = 0; j < data.data.length; j++) {
                ...
                var comment_c = data.data[j].comment_count;
                console.log(comments);
                console.log(comment_c);
                var homework = "\n<li>\n    <div class=\"message\">\n        <p class=\"teacher\"><span class=\"yu\">" + coursefn(data.data[j].course) + "</span><span class=\"jiang\">" + 
                                data.data[j].publisher.name + "老师布置了作业</span><span class=\"date\">" + datefn(data.data[j].publish_at) + "</span></p>\n  <div class=\"work-in\">\n <p class=\"start\">\n " +
                                " <span class=\"classify neirong\">作业内容</span><span class=\"definite\">" + homeworkFn(data.data[j].content,data.data[j].images) + "</span><div class=\"clear\"></div></p>\n ";
               if(data.data[j].record_length > 0 ){
                    homework += " <p><span class=\"classify voice\">语音</span>" + homeworkAud(data.data[j].record_length,data.data[j].record_url) + "</p>\n";
               }
                   homework +=  "<p class=\"end\"><span class=\"classify\">截止时间</span><span class=\"end-date\">" + 
                                datefn(data.data[j].deadline) + "</span></p>\n  </div>\n <p class=\"comment\">共" + comment_c +"条评论</p>\n <div class=\"comment-in\">\n  <input type=\"text\" placeholder=\"说两句吧\" data-container=\"body\" data-toggle=\"popover\" data-placement=\"bottom\"data-content=\"评论间隔不得小于5秒\"/>\n "+
            ...
            success: function (data) {
                ...
                if (data.data.length>0) {
                    ...
                    for (var i = 0; i < data.data.length; i++) {
                        ...
                        var myClass = ("\n<li >\n    <div class=\"class-txt\">\n        <p class=\"schools\">" + data.data[i].school.school_name + fn(data.data[i].grade) + data.data[i].name + "</p>\n        <p><span>班号："+data.data[i].id +"</span><span class=\"copy\">   复制</span></p>\n       <p>教师：" + classTnfn(data.data[i].classmembers) + "【教号:" + classTfn(data.data[i].classmembers) + "】</p>\n    </div>\n</li>\n");
                        $(".class ul").append(myClass);
                    }
                    var classrooms_ids = classrooms_id_group.join();
                    // getNotices(classrooms_ids,userId,session);
                    // getMaterial(classrooms_ids,userId,session);
                    // getHomework(classrooms_ids,userId,session);
                }
            },
            error: function (data) {
                // console.log(data);
...
// 添加班级相关方法
    ...
    function getMaterial(classrooms_ids,user_id,sess){
        ...
        //         contentType: "application/json;charset=UTF-8",
        //         beforeSend: function (request) {
        //             request.setRequestHeader("X-Session", sess);
        //         },
        //         dataType: "json",
        //         success: function (data) {
        //             console.log(data);
        ...
        //                 console.log(data.data[0]);
        //                 for (var i = 0; i < data.data.length; i++) {
        //                     var mat = ("\n<li>\n  <div class=\"courseware-in\">\n      <p class=\"title\">" + data.data[i].title + "<i></i></p>\n      <p>" + data.data[i].url + "</p>\n      <p class=\"date\">" + data.data[i].publish_at + "</p>\n  </div>\n</li>\n");

        //                     $(".courseware ul").append(mat);
        //                 }
        //             }
        //         }
        //     });
        ...
        case 'homework':
            ...
            var options = {
             ...
             beforeSubmit:function(){
               //alert("表单提交前执行的操作！");
               var imgFil = $("#file2 input[name=file]");
               var voiceFil = $('#file2 input[name=voice]');
               console.log(imgFil);
               // 判断是否为图片格式
               var format = imgFil.val().split(".")[1];
               // 判断是否为音频格式
               var voFormat = voiceFil.val().split(".")[1];
               console.log(format);
               //if(imgFil.val() == ""){
               //   console.log("不能为空");
               ...
               if(format && format !=="jpg" && format!=="gif" && format !=="png" && format !=="jpeg"){
                    imgFil.next().text('不支持的图片格式');
                    return false;
               }

             if(voFormat && voFormat !== "acc" && voFormat !=="mp3" && voFormat !="wav" && voFormat !="flac" && voFormat !="ogg" && voFormat !="wma"){
                 voiceFil.next().text('不支持的音频格式');
                 return false;
             }
             ...
             success:function(json){//表单提交成功回调函数
              ...
              console.log(json);
              jsonObjH["images"] = json.images;
              jsonObjH['record_url'] = json.voice;
               ...
               $.ajax({
                  ...
                  success: function (data) {
                    homeForm.modal('hide')
                       console.log(data);
                      if (data.data.length>0) {
                        var homework = "\n<li>\n <div class=\"message\">\n  <p class=\"teacher\"><span class=\"yu\">" + coursefn(data.data[0].course) + 
                             "</span><span class=\"jiang\">" + data.data[0].publisher.name + "老师布置了作业</span><span class=\"date\">" + datefn(data.data[0].publish_at) + 
                             "</span></p>\n  <div class=\"work-in\">\n  <p class=\"start\">\n  <span class=\"classify neirong\">作业内容</span><span class=\"definite\">" + 
                             homeworkFn(data.data[0].content) + "</span><div class=\"clear\"></div></p>\n"
                            if(data.data[0].record_length > 0){
                                 homework +="<p><span class=\"classify voice\">语音</span>" + homeworkAud(data.data[0].record_length,data.data[0].record_url) + "</p>\n";
                            }
                           homework +=   "<p class=\"end\"><span class=\"classify\">截止时间</span><span class=\"end-date\">" + 
                             datefn(data.data[0].deadline) + "</span></p>\n </div>\n <p class=\"comment\">共1049条评论</p>\n <div class=\"comment-in\">\n" + 
                             " <input type=\"text\" placeholder=\"说两句吧\" data-container=\"body\" data-toggle=\"popover\" data-placement=\"bottom\"data-content=\"评论间隔不得小于5秒\"/>\n "+
                             " <span class=\"button sendMess " +data.data[0].id +"\">发送</span><div sytle=\"clear:both;\"></div>";

                            homework += "<ul class=\"comments\"></ul>";

                        $("ul.homework").prepend(homework);
