...
function getClasses() {
    if ($preSchool) {
        $.ajax({
            type: "GET",
            url: "../classrooms?school_id=" + $preSchool.attr("id").split('-')[0],
            contentType: "application/json;charset=UTF-8",
            dataType: "json",
            success: function (data) {
                if (data.meta.code == 200) {
...
function joinClass(type) {
    ...
    $.ajax({
        type: "POST",
        url: "../classrooms/" + $("#classId").val() + "/members",
        contentType: "application/json;charset=UTF-8",
        beforeSend: function (request) {
            request.setRequestHeader("X-Session", session);
        },
...
function verifyAuthCode(type, phoneNumber, smsCode, $errMsg, userInfo) {
    $.ajax({
        type: "GET",
        url: "../verify?phone=" + phoneNumber + "&sms_code=" + smsCode,
        contentType: "application/json;charset=UTF-8",  //发送信息至服务器时内容编码类型。
        dataType: "json",   //预期服务器返回的数据类型。如果不指定，jQuery 将自动根据 HTTP 包 MIME 信息来智能判断，比如XML MIME类型就被识别为XML。
        success: function (data) {
            if (data.code == 200) {
